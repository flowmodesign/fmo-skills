...
- Only `import React from "react"` is supported; default-export a function component. CSS you pass is auto-scoped to the host. Style and query inside the host by **class**, never by `id` — the host can be duplicated on the canvas, and a second copy of an id-keyed rule or `getElementById` lookup silently drives the first instance. Keep refs (`useRef`) or classes for anything the component addresses itself. Use exposed `fields` (text/number/color/boolean/select/image/url/range/repeater) for anything the user should tweak from the sidebar.
